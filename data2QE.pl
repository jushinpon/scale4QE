...
for my $id (@datafile){
  ...
  #  chomp ($species,$starting_magnetization);
    ...
    $data_path =~ s/^\s+|\s+$//g;
    #get kpoint,starting_magnetization, and species from ori.in
    my $kpoint = `grep -v '^[[:space:]]*\$' $data_path/ori.in|grep -A 1 K_POINTS|grep -v K_POINTS|grep -v -- '--'`;
    die "kpoint is not got for $id\n" unless($kpoint);
    
    my @start_mag = `grep -v '^[[:space:]]*\$' $data_path/ori.in|grep starting_magnetization|grep -v -- '--'`;
    map { s/^\s+|\s+$//g; } @start_mag;
    die "starting_magnetization is not catched for $id\n" unless(@start_mag);
    my $start_mag = join("\n",@start_mag);
   # print "\$start_mag: $start_mag\n";
   # die;
    my @SPECIES = `grep -v '^[[:space:]]*\$' $data_path/ori.in|grep -A $current_typeNu ATOMIC_SPECIES| \\
                     grep -v ATOMIC_SPECIES|grep -v -- '--'`;
    map { s/^\s+|\s+$//g; } @SPECIES;
    die "ATOMIC_SPECIES is not catched for $id\n" unless (@SPECIES);
    my $SPECIES = join("\n",@SPECIES);
    chomp ($kpoint,$start_mag,$SPECIES);
...
#1 1 1.000000 4.458517505863 1.201338326940 0.873835074284 with charge
        ...
        #check all data
        die "no coords are captured in $id\n" unless (@{$para{coords}});
        my $coordNo = @{$para{coords}};
        die "Atom number are not equal in data and in QE in for $id\n" unless($atomNum ==$coordNo);
        ...
        my $cell = join("\n",($a,$b,$c));
        my $QEoutput_path = "$data_path/$data_name";
        `rm -rf $QEoutput_path`;
        `mkdir -p $QEoutput_path`;
        `rm -f $data_path/$data_name.in`;
        my $QEin = "$QEoutput_path/$data_name.in";
    ...
    my %QE_para = (
            ...
            pot_spec => $SPECIES,
            kpoint => $kpoint,
            starting_magnetization => $start_mag,
